...
- **Alur verifikasi berjenjang ke Atasan**: setiap karyawan punya **Posisi** (Atasan/Staff); akun **Staff** menunjuk satu **Atasan** (sesama karyawan berposisi Atasan) lewat Kelola Karyawan. Absen yang terindikasi mencurigakan otomatis masuk ke tab **Verifikasi** milik Atasan karyawan tsb (bukan cuma ke admin) untuk ditandai Terverifikasi/Ditolak — **Atasan wajib mengisi Catatan alasan** setiap kali memverifikasi/menolak. Admin (HCM) tetap bisa melakukan override lewat Riwayat Absensi dengan catatan yang sama wajibnya, dan semuanya tercatat di Log Aktivitas
...
- Dashboard admin: ringkasan, riwayat absensi (filter Nama & tanggal), kelola karyawan (tambah/edit/nonaktifkan dengan tanggal keluar, Posisi Atasan/Staff, dan penunjukan Atasan), kelola lokasi kantor, kelola jabatan, kelola hari libur, log aktivitas
...
- Heuristik fake-GPS bersifat indikatif (bukan bukti mutlak) — hasil "terindikasi" tetap tersimpan di riwayat untuk ditinjau Atasan (atau admin), absen tidak diblokir otomatis.
